...
async function sendMessage(message = inputMessage.value) {
  ...
  try {
    ...
    recommendedPlaces.value = Array.isArray(response.places)
      ? response.places
      : []
  ...
  <section :class="['chatbot-page', { 'is-started': hasStarted }]">
    <div class="chatbot-shell">
        ...
        <div :class="['accommodation-card', { selected: selectedAccommodation }]">
          <template v-if="selectedAccommodation">
            ...
            <button
              class="change-button"
              type="button"
              @click="showAccommodationModal = true"
            >
              숙소 변경
            </button>
          </template>
      ...
      <section v-if="hasStarted" class="map-panel" aria-label="추천 장소 지도">
        ...
        <div
          v-if="!recommendedPlaces.length && !isTyping"
          class="map-empty"
        >
