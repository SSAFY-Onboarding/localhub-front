...
function validate(input: PostCreateInput) {
  ...
  if (!title || title.length > 100)
    details.push({ field: 'title', reason: '제목은 1~100자로 입력해 주세요.' })
  if (!content || content.length > 5000)
    details.push({ field: 'content', reason: '내용은 1~5000자로 입력해 주세요.' })
  ...
  if (details.length)
    throw new ApiError('VALIDATION_ERROR', '입력값을 확인해 주세요.', 422, details)
...
export const postService = {
  async getPosts(page = 1, size = 10, keyword = ''): Promise<PostListResponse> {
    ...
    const posts = readPosts()
      .filter(
        (post) => !normalized || `${post.title} ${post.content}`.toLowerCase().includes(normalized),
      )
  ...
  async updatePost(id: number, input: PostUpdateInput): Promise<PostDetail> {
    ...
    if (!current || current.password !== input.password)
